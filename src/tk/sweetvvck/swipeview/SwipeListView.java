...
public class SwipeListView extends ListView {
	...
	public void dismiss(int position) {
		...
		if (height > 0) {
			...
			touchListener.closeOpenedItems();//TODO 实现删除item时，同时关闭所有打开的状态
		} else {
	 ...
	 *            Current position
	 */
	protected boolean onMove(int position, float x) {
		if (swipeListViewListener != null
				&& position != ListView.INVALID_POSITION) {
			return swipeListViewListener.onMove(position, x);
		}
		return false;
	}
	 ...
	 * @param downPosition
	 */
	public void setOnDown(int downPosition) {
		if (swipeListViewListener != null) {
			swipeListViewListener.onDown(downPosition);
		}
	}

	/**
	 * 滑开swipeListview的一个item时关闭掉其他的item 于 2013-07-31 添加
	 * 
	 * @author 程科
	 */
	public void closeOtherItems() {
		touchListener.closeOtherItems();
	}

	/**
	 * 用于传出滑动列表时第一个visible的item的位置 于 2013-08-01 添加
	 * @author 程科
	 * @param firstPosition
	 */
	public void setOnScroll(int firstVisibleItem) {
		if (swipeListViewListener != null) {
			swipeListViewListener.onScroll(firstVisibleItem);
		}
	}
}
