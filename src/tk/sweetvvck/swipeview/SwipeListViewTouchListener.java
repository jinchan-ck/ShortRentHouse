...
public class SwipeListViewTouchListener implements View.OnTouchListener {
    ...
    private static final int DISPLACE_CHOICE = 200;
...
//            swipeListView.setItemChecked(position, !lastChecked);
     ...
     * @param position Position of list
     */
    private void closeAnimate4CloseOtherItems(View view, int position) {
    	if (opened.get(position)) {
    		generateRevealAnimate4CloseOtherItems(view, true, false, position);
    	}
    }
    ...
    /**
     * Create reveal animation
     * TODO 为实现滑开一个item则关闭其他所有item而实现的动画效果
     * 程科 添加于2013-08-01
     * @param view      affected view
     * @param swap      If will change state. If "false" returns to the original position
     * @param swapRight If swap is true, this parameter tells if movement is toward right or left
     * @param position  list position
     */
    private void generateRevealAnimate4CloseOtherItems(final View view, final boolean swap, final boolean swapRight, final int position) {
    	int moveTo = 0;
    	if (opened.get(position)) {
    		if (!swap) {
    			moveTo = openedRight.get(position) ? (int) (viewWidth - rightOffset) : (int) (-viewWidth + leftOffset);
    	...
    	.setListener(new AnimatorListenerAdapter() {
    		@Override
    		public void onAnimationEnd(Animator animation) {
    			//TODO 这里将这条注释掉，然后再当前item打开后调用，防止item还木有打开就卡住的bug
    			//swipeListView.resetScrolling();
    			if (swap) {
    				boolean aux = !opened.get(position);
    				opened.set(position, aux);
...
//    			resetCell();
    ...
    public AbsListView.OnScrollListener makeScrollListener() {
        return new AbsListView.OnScrollListener() {
            ...
            public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
            	/**
            	 * TODO 将firstVisibleItem传出去
            	 */
            	swipeListView.setOnScroll(firstVisibleItem); 
                if (isFirstItem) {
    ...
    /**
     * 程科 于 2013-07-31 添加
     * 滑开swipeListview的一个item时关闭掉其他的item
     */
    void closeOtherItems() {
    	if (opened != null) {
    		int start = swipeListView.getFirstVisiblePosition();
    		int end = swipeListView.getLastVisiblePosition();
    		for (int i = start; i <= end; i++) {
    			if (opened.get(i)) {
    				if(downPosition != i)
    					closeAnimate4CloseOtherItems(swipeListView.getChildAt(i - start).findViewById(swipeFrontView), i);
    			}
    		}
    	}
    }
    ...
    public boolean onTouch(View view, MotionEvent motionEvent) {
        ...
        switch (motionEvent.getActionMasked()) {
            case MotionEvent.ACTION_DOWN: {
                ...
                swipeListView.setOnDown(downPosition);
            ...
            case MotionEvent.ACTION_UP: {
                ...
                //TODO 这里修改为宽度的1/4就滑开或关闭item
                else if (Math.abs(deltaX) > viewWidth / 6) {
                    swap = true;
                    swapRight = deltaX > 0;
                }
                generateAnimate(frontView, swap, swapRight, downPosition);
                if (swipeCurrentAction == SwipeListView.SWIPE_ACTION_CHOICE && deltaX >= viewWidth/DISPLACE_CHOICE && downPosition >=0) {
...
//                }
    ...
    public void move(float deltaX) {
        boolean canDragRight = swipeListView.onMove(downPosition, deltaX);
        if(canDragRight && deltaX >0){
        	return;
        }
        ...
        if (swipeCurrentAction == SwipeListView.SWIPE_ACTION_DISMISS) {
            ...
            if ((swipingRight && deltaX > 0 && posX < DISPLACE_CHOICE)
                    ...
                    || (!swipingRight && deltaX > -DISPLACE_CHOICE)) {
                setTranslationX(frontView, deltaX/(viewWidth/DISPLACE_CHOICE));
                //TODO 暂时隐藏标记为已读的勾
                //setAlpha(parentView.findViewById(R.id.read_img), (deltaX/viewWidth));
            }
        } else {
